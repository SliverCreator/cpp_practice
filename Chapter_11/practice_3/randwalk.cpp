...
#include <fstream>
#include "vector.h"    // 包含自定义的Vector类
using namespace std;
using VECTOR::Vector;  // 使用VECTOR命名空间中的Vector类
...
int main() {
    ...
    Vector step;        // 表示每一步的向量
    Vector result(0.0, 0.0);  // 初始化结果向量，起始位置为原点
    ...
    for (int i = 0; i < run_times; i++) {
        ...
        while (result.magval() < target) {  // 当当前位置到原点的距离小于目标距离时继续
            ...
            step.reset(dstep, direction, Vector::POL);  // 使用极坐标设置步长和方向
    ...
    cout << "Average steps: " << (run_times > 0 ? (sum_steps / run_times) : 0) << endl;
